...
mat4 m=glm::transpose(mat4(projection_mat))*view_mat;
...
void printPlanesData()
{
    for (int i = 0; i < planes.size(); ++i) {
        ...
        std::cout<<("equation:" + std::to_string(planes[i].normal.x) + "a+" + std::to_string(planes[i].normal.y) + "b+" +
...
int main() {
    ...
    std::cout<<"projection matrix transposed:\n"<<get_matrix_sting(glm::transpose(mat4(projection_mat)))<<std::endl<<std::endl;
    ...
    std::cout<<"transposed projection*view:\n"<<get_matrix_sting(glm::transpose(mat4(projection_mat))*view_mat)<<std::endl<<std::endl;
